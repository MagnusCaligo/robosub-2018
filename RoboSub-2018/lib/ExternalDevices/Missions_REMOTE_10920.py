...
class AbstractMission(QtCore.QObject):
    ...
    def __init__(self, parameters):
        ...
        self.specificDistanceError = 1 #How much error we want on the specific waypoint
        self.specificRotationError = 10#how much rotation error we want on the specific waypoint
...
class Buoys(AbstractMission):
    def __init__(self, parameters):
        ...
        self.angle = 10#Angle to rotate by when looking for obstacles
    ...
    def lookForObstacles(self, externalComm, movementController):
		...
		if self.parameters["useNeuralNetwork"] == "True":
			...
			if self.relativeMoveWaypoint == None:
				...
				error = movementController.advancedMove(externalComm.orientation + externalComm.position, self.relativeMoveWaypoint[0], self.relativeMoveWaypoint[1], externalComm.position[2],
    ...
    def calculateSpecificWaypoint(self, externalComm, movementController):
        ...
        if self.relativeMoveWaypoint != None:   
                ...
                #Check to see if we are the correct orientation
				if abs(error[3]) < self.generalOrientationError and abs(error[4]) < self.generalOrientationError and abs(error[5]) < self.generalOrientationError:
					pass #Only called if we are at the correct orientation
				else:
					reachedWaypoint = False
				...
				if reachedWaypoint == True:
					if self.lookStartTime == None:
						self.lookStartTime = time.time()
					if time.time() - self.lookStartTime > self.lookTime:
						self.relativeMoveWaypoint = None
						self.lookStartTime = None
						self.generalWaypoint = externalComm.position + externalComm.orientation
						return True
        ...
        if len(detections) > 0:
            ...
            if tvec[1] > .5:
                ...
                goToDepth = -.5
            pose, n,e,u,x,y,z = movementController.relativeMove(externalComm.orientation+externalComm.position, None,None,None,None,None,None, tvec[0], tvec[1] - int(self.parameters["getDistanceAway"]), tvec[2], 0,0,0)
    ...
    def getExecutionPositionDifference(self, externalComm, movementController):
        ...
        if self.detections == detections and self.relativeMoveWaypoint != None:
            ...
            print tvec[0], tvec[1], tvec[2]
            #the tvec[2] is subtracted by 0 just to show that we should be trying to hit the buoy, as long as we don't get incorrect data from computer vision
            pose, n, e, u, x, y, z = movementController.relativeMove(externalComm.orientation + externalComm.position, None, None, None, None, None, None, tvec[0], tvec[1], tvec[2]-0, 0,0,0)
            self.relativeMoveWaypoint = [n, e, u, y, x, z]
            return False
            ...
            self.relativeMoveWaypoint = None
            self.resetFlagBoolean = True
            self.writeDebugMessage("Lost sight of obstacle, begining search")
    ...
    def getExecutionPositionDifference(self, externalComm, movementController):
    	...
    	if self.lookStartTime == None:
    		self.lookStartTime = time.time()
    	if time.time() - self.lookStartTime > 15: #Move forward for 15 seconds
    		self.readyToExecute = True
    		self.lookStartTime = None
    		
    	#Keep movine 1 feet forward until that ^ condition is met
    	movementController.relativeMove(externalComm.orientation + externalComm.position, None, None, None, None, None, None, 0,0,1,0,0,0)

    '''
    4. Tell the sub to ram into the buoy or pull the buoy.
    '''
    def executionTask(self):
...
class FootballGate(AbstractMission):
    ...
    def __init__(self, parameters):
        ...
        self.src_pts = [(0,0,0), (.5, 0,0), (.5, 4,0), (0, 4,0)]
    ...
    def lookForObstacles(self, externalComm, movementController):
		...
		if self.armsLocated == 2:
			...
			pose, n, e, u, x, y, z = movementController.relativeMove(externalComm.orientation + externalComm.position, None, None, None, None, None, None, xyz[0],xyz[1],xyz[2],0,0,0)
			self.relativeMoveWaypoint = [n,e,y,y,x,z]
...
class TorpedoBoard(AbstractMission):
	...
	def lookForObstacles(self, externalComm, movementController):
		...
		if self.relativeMoveWaypoint == None:
			pos, n, e, u, x, y, z = movementController.relativeMove(externalCopmm.orientation + externalComm.position, 0,0,self.depthOfObstacle - externalComm.position[2],0,self.angle,0)
		
			self.relativeMoveWaypoint = [n,e,u,y,x,z]
			self.angleChange += self.angle
			if abs(self.angleChange) > 360:
				self.angleChange %= 360
				print "Going down a foot"
				self.depthOfObstacle += 1
				if self.depthOfObstacle > 4:
					self.depthOfObstacle = 4	
				if self.relativeMoveWaypoint != None:
					error = movementController.advancedMove(externalComm.orientation + externalComm.position, self.relativeMoveWaypoint[0], self.relativeMoveWaypoint[1], externalComm.position[2], self.relativeMoveWaypoint[4], self.relativeMoveWaypoint[3], self.relativeMoveWaypoint[5])[1]
					#print error
					reachedWaypoint = True				
					#Check to see if we are at the correct location
					if abs(error[0]) < self.generalDistanceError and abs(error[1]) < self.generalDistanceError and abs(error[2]) < self.generalDistanceError:
					...
					if reachedWaypoint:
						if self.lookStartTime == None:
							...
							self.relativeMoveWaypoint = None
							self.lookStartTime = None
						#If the sub saw a board and has finished pointing towards it, then move on to next part of mission
							if self.movingToObstacles == True:
								print "Made it to the board"
								return True
	...
	def calculateSpecificWaypoint(self, externalComm, movementController):
		...
		if len(squidDetections) > 0 or len(tentacleDetections) > 0:
			...
			pose, n, e, u, x,y,z = movementController.relativeMove(externalComm.orientation + externalComm.position, None, None, None, None, None,None, 0,0,0,tvec[0],tvec[1],tvec[2] - int(self.parameters["getDistanceAway"]))
			self.relativeMoveWaypoint = [n,e,u,y,x,z]
	...
	def getExecutionPositionDifference(self,externalComm, movementController):
		...
		while(orientationCorrect == False):
			...
			if(self.parameters["torpedoBoard"] == 0): #We're looking for the squid board
				if(self.squidCorners[0] != None):
					topLeftCorner = squidCorners[0]
					numOfCorners += 1
				if(self.squidCorners[1] != None):
					topRightCorner = squidCorners[1]
					numOfCorners += 1
				if(self.squidCorners[2] != None):
					bottomRightCorner = squidCorners[2]
					numOfCorners += 1
				if(self.squidCorners[3] != None):
					bottomLeftCorner = squidCorners[3]
					numOfCorners += 1
				if(numOfCorners == 1):
					for i in enumerate(self.squidCorners):
						if(self.squidCorners[i] != None):
							if(self.squidCorners[i][0] < 404 & self.squidCorners[i][1] < 304): # Top left quadrant
								pass # MOVE up and left
							if(self.squidCorners[i][0] > 404 & self.squidCorners[i][1] < 304): # Top right quadrant
								pass # MOVE up and right
							if(self.squidCorners[i][0] > 404 & self.squidCorners[i][1] > 304): # Bottom right quadrant
								pass # MOVE down and right
							if(self.squidCorners[i][0] < 404 & self.squidCorners[i][1] > 304): # Bottom left quadrant
				...
				if(self.tentacleCorner[0] != None):
					topLeftCorner = squidCorners[0]
					numOfCorners += 1
				if(self.tentacleCorner[1] != None):
					topRightCorner = squidCorners[1]
					numOfCorners += 1
				if(self.tentacleCorner[2] != None):
					bottomRightCorner = squidCorners[2]
					numOfCorners += 1
				if(self.tentacleCorner[3] != None):
					bottomLeftCorner = squidCorners[3]
					numOfCorners += 1
					
				if(numOfCorners == 1):
					for i in enumerate(self.tentacleCorners):
						if(self.tentacleCorners[i] != None):
							if(self.tentacleCorners[i][0] < 404 & self.tentacleCorners[i][1] > 304): # Top left quadrant
								pass # MOVE up and left
							if(self.tentacleCorners[i][0] > 404 & self.tentacleCorners[i][1] > 304): # Top right quadrant
								pass # MOVE up and right
							if(self.tentacleCorners[i][0] > 404 & self.tentacleCorners[i][1] < 304): # Bottom right quadrant
								pass # MOVE down and right
							if(self.tentacleCorners[i][0] < 404 & self.tentacleCorners[i][1] < 304): # Bottom left quadrant
								pass # MOVE down and left
